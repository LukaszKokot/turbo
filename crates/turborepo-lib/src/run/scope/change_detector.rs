...
impl<'a> SCMChangeDetector<'a> {
    ...
    fn lockfile_changed(
        ...
        let lockfile_path_relative = turbo_root
            ...
            .map_err(|_| ChangeDetectError::LockfileNotInRepo)?;
...
pub enum ChangeDetectError {
    ...
    /// This is a potential case that may arise in Bun. We may need to
    /// relax / handle this case in the future
    #[error("Lockfile not in repo")]
    LockfileNotInRepo,
}
